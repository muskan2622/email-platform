      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <GlassCard className="p-4 md:p-6">
          <div className="mb-6 flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-white">Incoming events</span>
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="font-mono text-sm text-white">{ev.type}</p>
                        ...
                        <ChevronDown
                          className={`h-4 w-4 text-white/40 transition-transform ${expanded === ev.id ? "rotate-180" : ""}`}
                        />
                      </button>
                      <AnimatePresence>
                        {expanded === ev.id && (
                          <motion.pre
                            ...
                            className="overflow-hidden border-t border-white/[0.06] bg-black/40 px-4 py-3 font-mono text-xs text-cyan-200/80"
                          >
        ...
        <GlassCard className="p-5">
          <h3 className="mb-4 text-sm font-medium text-white">Processing pipeline</h3>
            ...
            <div className="relative space-y-6 pl-4 before:absolute before:left-[5px] before:top-2 before:h-[calc(100%-8px)] before:w-px before:bg-gradient-to-b before:from-violet-500 before:to-transparent">
              {selected.processing_result.evaluations.map((ev, i) => (
                <motion.div
                  ...
                  className="relative"
                >
                  <span className="absolute -left-4 top-1 h-2.5 w-2.5 rounded-full border-2 border-violet-400 bg-[#030306]" />
                  <p className="text-sm text-white/80">{ev.trigger_name}</p>
                  <p className="text-[10px] text-white/35">{ev.status}{ev.skip_reason ? ` · ${ev.skip_reason}` : ""}</p>
                </motion.div>
              ))}
            </div>
          ) : (
