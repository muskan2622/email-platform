-- Pulsemail: wizard-based automation builder schema

-- ---------------------------------------------------------------------------
-- Automations (wizard-created flows)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Condition groups (normalized tree for automations)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Individual conditions (human-friendly labels stored for UI replay)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Actions (send email, webhooks, etc.)
-- ---------------------------------------------------------------------------
...
create policy "Authenticated users manage automations"
  on public.automations for all to authenticated using (true) with check (true);

create policy "Authenticated users manage automation_condition_groups"
  on public.automation_condition_groups for all to authenticated using (true) with check (true);

create policy "Authenticated users manage automation_conditions"
  on public.automation_conditions for all to authenticated using (true) with check (true);

create policy "Authenticated users manage automation_actions"
  on public.automation_actions for all to authenticated using (true) with check (true);

create policy "Authenticated users read automation_runs"
